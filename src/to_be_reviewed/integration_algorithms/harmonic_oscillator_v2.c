...
int main() {
  ...
  if ((XandT = (posTime *)calloc(numPassi, sizeof(posTime))) == NULL) {
    fprintf(stderr, "ERRORE: non e' stato possibile allocare memoria, calcolo "
                    "del periodo non effettuabile.\n");
  }
...
void choiceAlg(param initParam, double dt, double tmax, posTime *XandT,
               int test) {
  unsigned int option;
  fprintf(stderr, "0 eulero-cromer, 1 per central point, 3 per velocity "
                  "verlet, 4 per RK4: ");
  scanf("%d", &option);
  if (option == RUNGE_KUTTA_4) {
    ...
    fprintf(stderr,
            "ERRORE: il numero inserito non corrisponde ad alcun algoritmo\n");
    exit(EXIT_FAILURE);
  }
}
...
param getData(double *dt, double *tmax, int *test) {
  ...
  fprintf(stderr, "v0: ");
  scanf("%lf", &out.v0);
  fprintf(stderr, "0 se vuoi effettuare test dell'errore, 1 se vuoi procedere "
                  "normalmente: ");
  scanf("%d", test);
  if (*test) {
