...
int determinante(int matrix[NR][NC], int nr, int nc)
{
  ...
  {
    return matrix[0][0] * matrix[1][1] * matrix[2][2] + matrix[1][0] * matrix[2][1] * matrix[0][2] + matrix[0][1] * matrix[1][2] * matrix[2][0] - matrix[2][0] * matrix[1][1] * matrix[0][2] - matrix[2][1] * matrix[1][2] * matrix[0][0] - matrix[1][0] * matrix[0][1] * matrix[2][2];
  }
  else
  {
    for (i = nr - 1; i >= 0; i--)
    {
      ...
      det += pow(-1, i) * matrix[nr - 1][i] * determinante(compl, nr - 1, nr - 1);
    }
    return det;
  }
} 

void complementare(int matrix[NR][NC], int ordine, int colonna, int compl [NR - 1][NC - 1])
{
  int i, j;
  for (i = 0; i < ordine; i++)
  {
