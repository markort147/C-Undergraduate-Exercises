...
int main() {
  ...
  int i, numeroPassi, salto, numOsc, ripsmorz, optionForzato, tMax, nuoveCond,
      j;
  ...
  if (optionForzato == BACINO) {
    ...
    fprintf(fBacPos,
            "#Bacino di attrazione a velocita' positive\n#Parametri: t=%d "
            "f=%f\n#pos\tvel\n",
            tMax, inPar.ampForzante);
    fprintf(fBacNeg,
            "#Bacino di attrazione a velocita' negative\n#Parametri: t=%d "
            "f=%f\n#pos\tvel\n",
            tMax, inPar.ampForzante);
  }
  if (optionForzato == SEZIONE) {
    ...
    fprintf(fSez,
            "#Sezione di Poincare' a f=%f e periodo della forzante pari a "
            "%f\n#pos\t\tvel\n",
  ...
  do {
    if (ripsmorz == RIPSMORZ)
      inPar.cSmorzante +=
          0.01; /*per cercare il coefficiente di smorzamento critico*/
    ripsmorz = NORIPSMORZ;
    pendolo.x = inPar.xP;
    pendolo.v = inPar.vP;
    if (modo == DOPPIO) { /*energia iniziale*/
    ...
    numOsc = 0;
    j = 1;
    for (i = 0; i < numeroPassi + 1; i++) { /*integrazione*/
      pendolo.t = i * dt;
      if (modo == DOPPIO) { /*per pendolo doppio*/
        pendolo2.t = i * dt;
        E = energiaDoppio(pendolo, pendolo2);
        erroreEnergia = (long double)(E - E0) / E0;
        printf("%lf\t%lf\t%lf\t%lf\t%lf\t%Lf\n", pendolo.t, pendolo.x,
               pendolo2.x, pendolo.v, pendolo2.v, erroreEnergia);
        rungeKutta4Doppio(&pendolo, &pendolo2, dt);
      } else { /*per pendoli non-doppi*/
        check = pendolo;
        E = energia(pendolo);
        erroreEnergia = (long double)(E - E0) / E0;
        printf("%lf\t%lf\t%lf\t%Lf\n", pendolo.t, pendolo.x, pendolo.v,
               erroreEnergia);
        pendolo = rungeKutta4(pendolo, dt);
        /*campionamento per la sezione di poincare'*/
        if (optionForzato == SEZIONE &&
            (periodoCampionamento * j) >= pendolo.t &&
            (periodoCampionamento * j) < ((i + 1) * dt)) {
          fprintf(fSez, "%lf\t%lf\n", pendolo.x, pendolo.v);
          j++;
        }
        if (salto == 0) { /*per cercare i semiperiodi*/
        ...
        } else { /*salto per evitare di prendere due volte lo stesso zero*/
          salto = 0;
        }
      }
    }
    ...
    if (modo == SMORZATO) {
      fprintf(stderr,
              "Con coefficiente di smorzamento %.2lf, il pendolo compie %d "
              "oscillazioni.\nRipetere incrementando il coefficiente di 0.01? "
              "(0=n,1=y):",
              inPar.cSmorzante, numOsc);
      scanf("%d", &ripsmorz);
    }
  } while ((modo == SMORZATO && ripsmorz == RIPSMORZ) ||
           (optionForzato == BACINO && nuoveCond));
  fclose(fperiod);
  if (optionForzato == BACINO) {
    fclose(fBacNeg);
    fclose(fBacPos);
...
double
energiaDoppio(cinematica in,
              cinematica in2) { /*nel calcolo dell'errore sull'energia è
                                   irrilevante la massa, che si semplifica*/
  double V, K;
  K = 0.5 * inPar.r * inPar.r *
      (2 * in.v * in.v + in2.v * in2.v + 2 * in.v * in2.v * cos(in.x - in2.x));
  V = -inPar.r * inPar.g * (2 * cos(in.x) + cos(in2.x));
  return V + K;
}

double energia(cinematica in) { /*nel calcolo dell'errore sull'energia è
                                   irrilevante la massa, che si semplifica*/
  double V, K;
...
void sceltaModo() {
  do {
    fprintf(stderr,
            "%d)Pendolo semplice;\n%d)pendolo smorzato;\n%d)pendolo smorzato "
            "sottoposto a forzante periodica;\n%d)pendolo doppio.\n:",
            SEMPLICE, SMORZATO, FORZATO, DOPPIO);
    scanf("%d", &modo);
  } while (modo != DOPPIO && modo != SEMPLICE && modo != SMORZATO &&
           modo != FORZATO);
  if (modo == SEMPLICE)
...
double accelForz(cinematica pendolo) {
  return accelSmorz(pendolo) +
         inPar.ampForzante * cos(inPar.frForzante * pendolo.t);
}
...
double accelDoppio(cinematica pendolo, cinematica pendolo2) {
  ...
  cosDiff = cos(diff);
  num =
      sin(diff) * (pendolo.v * pendolo.v * cosDiff + pendolo2.v * pendolo2.v) +
      inPar.g * (2 * sin(pendolo.x) - sin(pendolo2.x) * cosDiff) / inPar.r;
...
double accelDoppio2(cinematica pendolo, cinematica pendolo2) {
  ...
  cosDiff = cos(diff);
  num = sin(diff) *
            (pendolo.v * pendolo.v + 0.5 * pendolo2.v * pendolo2.v * cosDiff) +
        inPar.g * (sin(pendolo.x) * cosDiff - sin(pendolo2.x)) / inPar.r;
