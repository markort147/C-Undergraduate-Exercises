...
int main () {
  ...
  double dt, tmax;
  osc = getData (&dt,&tmax);
  fprintf(stderr,"0 per eulero, 1 per eulero-cromer, 2 per central point, 3 per leap frog, 4 per velocity verlet, 5 per RK4: ");
  scanf("%d",&option);
  if(option==5) {rk4(osc,dt,tmax);}else if(option==4) {velVerlet(osc,dt,tmax);}
  else  if(option==3) {leapFrog(osc,dt,tmax);}else if(option==2) {cPoint(osc,dt,tmax);} 
  else if(option==0) {euler(osc,dt,tmax);} else if(option==1) {eulercrom(osc,dt,tmax);}
  return 0;
}/*main*/
...
struct param getData (double*dt,double*tmax) {
  double p;
  struct param out;
  fprintf(stderr,"Massa: "); scanf("%lf",&out.m);
  fprintf(stderr,"k: "); scanf("%lf",&out.k);
  p = 2*M_PI*sqrt(out.m/out.k);
  fprintf(stderr,"x0: "); scanf("%lf",&out.x0);
  fprintf(stderr,"v0: "); scanf("%lf",&out.v0);
  fprintf(stderr,"Periodo: %lf. Delta t consigliato: tra %lf e %lf.\n",p,p/20,p/100);
  fprintf(stderr,"dt: "); scanf("%lf",dt);
  fprintf(stderr,"tmax: "); scanf("%lf",tmax);
  return out;
}/*getData*/

double accel (double k,double x,double m) {
  return -k*x/m;
}/*accel*/
...
void rk4 (struct param in,double dt,double tmax) {
  ...
  E0 = v*v*in.m/2 - in.k*x1*x1/2;
  for(i=0;i<N+1;i++) {
    k1 = v; l1 = accel(x1,in.k,in.m);
    k2 = v + l1*dt/2; l2 = accel(x1+k1*dt/2,in.k,in.m);
    k3 = v + l2*dt/2; l3 = accel(x1+k2*dt/2,in.k,in.m);
    k4 = v + l3*dt; l4 = accel(x1+k3*dt,in.k,in.m);
