...
int main (int argc,char* argv[]) {
  ...
  if ( (data=(int*)malloc(numEvents*sizeof(int))) == NULL) {
    fprintf(stderr,"ERRORE: non sono riuscito ad allocare memoria per l'array 'data'.\n");
  }
  fprintf(stderr,"0)# di bin pari alla radice quadrata del # di eventi;\n1)# di bin da inserire manualmente.\n: ");
  scanf("%d",&option);
  if(option==1) {
  ...
  for(i=0;i<numEvents;i++) {
    for(j=0;j<numBins;j++) {
      if(data[i]>=data[iMin]+j*binSize && data[i]<data[iMin]+(j+1)*binSize) freq[j]++;
    }
  }
  fprintf(foutput,"%d\t 0\n",data[iMin]);
  for(i=0;i<numBins;i++) {
    fprintf(foutput,"%lf\t%lf\n%lf\t%lf\n",(data[iMin]+i*binSize),freq[i]/(double)numEvents,(data[iMin]+(i+1)*binSize),freq[i]/(double)numEvents);
  }
  fprintf(foutput,"%d\t 0\n",data[iMax]);
  fclose(finput);
  fclose(foutput);  
