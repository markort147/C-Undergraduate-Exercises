...
int main()
{
  ...
  srand(time(0));
  /*intro*/
  printf("Data una sfera di raggio R=%d e %d atomi di un gas perfetto, questo programma calcola quale frazione di atomi si trova nella corona sferica inclusa tra due sfere di raggio R1 e R2, tali che R1<R2<R.\n", RMAX, ATOM);
  /*acquisisco raggi*/
  do
  {
...
void fillPrintDist(double pos[ATOM][3], double *distance)
{
  ...
  {
    printf("Impossibile aprire il file ''distanze.dat''. Il programma verrà terminato.\n");
    exit(EXIT_FAILURE);
  }
  for (i = 0; i < ATOM; i++)
  {
