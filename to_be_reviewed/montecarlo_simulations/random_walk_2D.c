...
void makeHist() {
  ...
  FILE *pHistX, *pHistY;
  if ((pHistX = fopen("rw2D_hist_x.dat", "w")) == NULL ||
      (pHistY = fopen("rw2D_hist_y.dat", "w")) == NULL) {
    fprintf(
        stderr,
        "ERROR: could not open ''rw2D_hist_x.dat'' or ''rw2D_hist_y.dat''\n");
    exit(EXIT_FAILURE);
  }
  for (i = 0; i < TRAJECT; i++) {
    for (j = 0; j < BINS; j++) {
...
void printAverage() {
  ...
  fprintf(pAverage,
          "#t\t average(X)\t average(Y)\t average(X^2)\t average(Y^2)\n");
  for (i = 0; i < AVERAGE_LEN; i++) {
    fprintf(pAverage, "%d\t %f\t %f\t %f\t %f\n", (i + 1) * AVERAGE_STEP,
            (averagePos + i)->x, (averagePos + i)->y, (averageSqPos + i)->x,
            (averageSqPos + i)->y);
  }
  fclose(pAverage);
}
