...
/*getting data*/
void getData(stars *mass, coords *pos, coords *vel, int *tMax, double *dt,
             int *fr, int test) {
  int option;
  fprintf(stderr,
          "Choice option: \n0) M_A=0, M_B=1, x_0=2.9, y_0=0, v_x=0, v_y=2*PI; "
          "\n1) M_A=1, M_B=1, x_0=2.9, y_0=0, v_x=0, v_y=2*PI; \n2) M_A=0, "
          "M_B=1, x_0=1.5, y_0=1, v_x=-2*PI, v_y=0; \n3) M_A=1, M_B=1, "
          "x_0=1.5, y_0=1, v_x=-2*PI, v_y=0; \n4) M_A=1, M_B=1, x_0=2.5, "
          "y_0=0, v_x=0, v_y=2*PI; \n5) manual; \n: ");
  scanf("%d", &option);
  if (option == 0) {
    mass->a = 0;
    mass->b = 1;
  ...
  if (test == NTEST) {
    ...
    scanf("%lf", dt);
    fprintf(stderr,
            "Gnuplot should plot N=%Ld points. What fraction N/n would you "
            "plot? Insert n: ",
            (long long int)((*tMax) / (*dt) + 0.5));
    scanf("%d", fr);
  }
}

/*simulation with velverlet*/
double simulation(stars mass, coords pos0, coords vel, int tMax, double dt,
                  int fr, int test) {
  int i, j;
  stars r;
  ...
  if (test == NTEST) {
    if (r.a <= MIN_DIST)
      fprintf(stderr,
              "Stars A annihilated the planet after %lf astronomical units of "
              "time.\n",
              dt * (i - 1));
    else if (r.b <= MIN_DIST)
      fprintf(stderr,
              "Stars B annihilated the planet after %lf astronomical units of "
              "time.\n",
              dt * (i - 1));
  }
  return dE / E_0;
...
stars dist(coords pos) {
  stars r;
  r.a = sqrt(pos.x * pos.x + pos.y * pos.y + pos.x * DELTA +
             DELTA * DELTA * 0.25);
  r.b = sqrt(pos.x * pos.x + pos.y * pos.y - pos.x * DELTA +
             DELTA * DELTA * 0.25);
  return r;
}
...
coords accel(coords pos, stars r, stars mass) {
  ...
  acc.x =
      K * (ratio.a + ratio.b) * pos.x + K * 0.5 * DELTA * (ratio.a - ratio.b);
...
double separation(coords pos, coords vel, stars r, stars mass) {
  return pow((pos.x * vel.y - pos.y * vel.x), 2) +
         vel.x * vel.x * DELTA * DELTA * 0.25 -
         K * pos.x * DELTA * (mass.a / r.a - mass.b / r.b);
}
